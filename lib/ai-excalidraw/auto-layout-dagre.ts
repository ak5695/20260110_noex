...
export type LayoutStrategy = 'mindmap' | 'flowchart' | 'architecture' | 'venn' | 'dataviz' | 'freeform';

/**
 * Apply specific layout strategy
 */
export function layoutWithStrategy(
    elements: CompactElement[],
    strategy: LayoutStrategy = 'flowchart'
): CompactElement[] {
    console.log(`[Layout] Applying strategy: ${strategy}`);

    switch (strategy) {
        case 'architecture':
            return layoutWithDagre(elements, { rankdir: 'LR', ranksep: 150, nodesep: 50 });

        case 'mindmap':
            // Use specialized radial layout
            return layoutRadial(elements);

        case 'venn':
            // Fixed circle layout
            return layoutVenn(elements);

        case 'dataviz':
            // Axis aligned (Grid essentially)
            return layoutGrid(elements, false);

        case 'freeform':
            // Grid + Random
            return layoutGrid(elements, true);

        case 'flowchart':
        default:
            return layoutWithDagre(elements, { rankdir: 'TB' });
    }
}
 ...
 */
function layoutRadial(elements: CompactElement[]): CompactElement[] {
    // 1. Identify root (node with most connections OR no parents)
    // Simple heuristic: First node is root? Or node with ID 'root'?
    ...
    elements.forEach(el => {
        if (['r', 'd', 'el', 'frame', 'text', 'fr'].includes(el.t) || TYPE_MAP[el.t]) allIds.add(el.i);
        if (el.t === 'a' || el.t === 'arrow') {
            if (!connections[el.si!]) connections[el.si!] = [];
            connections[el.si!].push(el.ei!);
            parents[el.ei!] = el.si!;
        }
    });

    // Find root: Node with no parent, or first node
    let rootId = Array.from(allIds).find(id => !parents[id]) || elements[0]?.i;

    if (!rootId) return elements;

    const CENTER_X = 0;
    const CENTER_Y = 0;
    const LEVEL_RADIUS = 300; // Distance between rings

    // recursive placement
    const placed = new Set<string>();

    const placeNode = (id: string, level: number, startAngle: number, endAngle: number) => {
        const el = elements.find(e => e.i === id);
        if (!el || placed.has(id)) return;

        const angle = (startAngle + endAngle) / 2;
        const radius = level * LEVEL_RADIUS;

        let cx = CENTER_X + Math.cos(angle) * radius;
        let cy = CENTER_Y + Math.sin(angle) * radius;

        // Root is always center
        if (level === 0) { cx = CENTER_X; cy = CENTER_Y; }
        ...
        const children = connections[id] || [];
        if (children.length > 0) {
            const spanPerChild = (endAngle - startAngle) / children.length;
            children.forEach((childId, index) => {
                const s = startAngle + index * spanPerChild;
                const e = s + spanPerChild;
                placeNode(childId, level + 1, s, e);
            });
        }
    };

    placeNode(rootId, 0, 0, Math.PI * 2);

    // Position floating nodes (not connected)
    let floatIndex = 0;
    elements.forEach(el => {
        if (!placed.has(el.i) && el.t !== 'a' && el.t !== 'arrow') {
            el.x = -500;
            el.y = floatIndex * 100;
            floatIndex++;
        }
...
/**
 * Venn Diagram Layout (Fixed Circles)
 * Heuristic: Max 3 main sets.
 */
function layoutVenn(elements: CompactElement[]): CompactElement[] {
    const nodes = elements.filter(el => el.t !== 'a' && el.t !== 'arrow');
    ...
    nodes.forEach((el, index) => {
        ...
        // Force circle shape if not strictly defined? No, keep user type.
        // But making them larger ensures overlap visualization if they are big.

        const width = el.w || 200; // Force larger size for Venn look
        const height = width;
        ...
        // If it's a Venn, usually we want ellipses or circles.
        if (el.t === 'r') el.t = 'el';
    });
...
/**
 * Grid Layout (Dataviz / Freeform)
 */
function layoutGrid(elements: CompactElement[], useJitter: boolean): CompactElement[] {
    const nodes = elements.filter(el => el.t !== 'a' && el.t !== 'arrow');
    ...
    nodes.forEach((el, index) => {
        ...
        const width = el.w || 150;
        const height = el.h || 100;
        el.w = width;
        el.h = height;

        let x = START_X + col * (width + GAP);
        let y = START_Y + row * (height + GAP);
